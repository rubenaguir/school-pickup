...
| `asApiError` (normaliza un `catch (caught: unknown)` a `ApiError`) está duplicada en ~17 archivos de los 3 frontends — mucho más allá de las 5 del patrón WS de ADR-075 | Detectado al escribir el ADR-075 punto 2 (Paso 2): promovida a `packages/shared/src/api-client/api-error.ts`, pero solo migrado su único consumidor de ese prompt (`useDeliveryPointQueue.ts`) — los otros 16 archivos siguen con su copia local, deliberadamente fuera de alcance | Migrar los 16 archivos restantes a la versión de `@casillego/shared` en un refactor aparte, sin ADR propio (es mecánico: reemplazar la función local por el import) — no mezclarlo con ningún cambio de comportamiento |
| `enrollments.grade_or_group` y `delivery_points.assigned_groups` son texto libre sin ningún vínculo entre sí (ADR-012) — renombrar o reconfigurar el grupo de un punto de entrega deja huérfanas en silencio a las matrículas que apuntaban al nombre anterior, sin aviso ni forma de detectarlo hoy | ADR-083 (observación de mantenimiento real del humano: reconfigurar puntos de entrega, ej. "1A" pasa a ser "2A" en otro punto) — ADR-012 ya sabía que tomaba este riesgo ("texto libre... para no bloquear altas de grupos nuevos antes de tener un catálogo curado"), este es el caso real que lo confirma | ADR-084 (pendiente, sin redactar): evaluar una entidad catálogo `institution_groups` con FK desde ambas columnas en vez de texto libre — cambio de modelo de datos real, dos entidades tocadas y migración, merece su propia sesión de spec-antes-de-código, no una nota suelta |
