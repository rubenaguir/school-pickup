 ...
 * states of the ADR-024 machine (`en_route`/`arriving`/`arrived`). Shared by
 * `SelectInstitution` (reroute after `ACTIVE_PICKUP_REQUEST_EXISTS`) and
 * `useActivePickupRequest` (the "Mis hijos" banner, ADR-092) so the two paths
 * never drift apart.
 */
export const ACTIVE_PICKUP_STATUSES = new Set<string>(['en_route', 'arriving', 'arrived']);
