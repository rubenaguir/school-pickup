...
import { acceptInvitationValidationError } from '../invitations/accept-invitation-rules';
...
/**
 * "Aceptar invitación" (ADR-082 punto 3), calco de `apps/parent`'s pantalla
 * homónima adaptado al layout `BrandPanel`/`Field`/`Alert` de este app (a
 * diferencia de los estilos inline de `apps/parent`, ver `VerifyEmail.tsx`).
          ...
          <div style={{ maxWidth: 380, width: '100%', margin: '0 auto' }}>
            ...
            {view === 'error' && terminalError && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <Alert
                  message={acceptInvitationErrorMessage(terminalError.code)}
...
function AcceptInvitationForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validation = acceptInvitationValidationError(password, confirmPassword);
    if (validation) {
  ...
  return (
    <>
      ...
      >
        <Field label="Nombre completo" htmlFor={fullNameId}>
          <input
            id={fullNameId}
            type="text"
            name="fullName"
        ...
        <Field label="Confirmar contraseña" htmlFor={confirmPasswordId}>
          <input
        ...
        {error && <Alert message={acceptInvitationErrorMessage(error.code)} code={error.code} />}
