...
import { boardMonitorTopic } from '@casillego/shared';
import { BoardMonitorGateway, type BoardMonitorWebSocket } from './board-monitor.gateway';
...
function buildGateway(overrides?: { verify?: unknown; checkMemberAccess?: unknown }) {
  ...
  const gateway = new BoardMonitorGateway(
    mqttClient as never,
...
function buildClient(): BoardMonitorWebSocket & { sent: string[]; closedWith: [number, string][] } {
  const sent: string[] = [];
...
function authorizedUrl(institutionId = INST_ID): string {
  return `/ws/board-monitor?accessToken=a.jwt.token&institutionId=${institutionId}`;
}

const BOARD_MONITOR_PAYLOAD = {
...
describe('BoardMonitorGateway', () => {
  describe('broker subscription', () => {
    it('subscribes once, on init, to the board-monitor wildcard topic', async () => {
  ...
  describe('connection authorization', () => {
    ...
    it('closes with 4400 INVALID_PAYLOAD when accessToken is missing', async () => {
      ...
      const client = await connect(gateway, `/ws/board-monitor?institutionId=${INST_ID}`);
    ...
    it('closes with 4400 INVALID_PAYLOAD when institutionId is missing', async () => {
      ...
      const client = await connect(gateway, '/ws/board-monitor?accessToken=a.jwt.token');
    ...
    // Carril is not a new role (ADR-071 pt.2): any institution_member, any
    // role, may open it — same authorization as BoardGateway, no `role` check.
    it('accepts a member regardless of role, mirroring BoardGateway', async () => {
      const { gateway, institutionAccess } = buildGateway({
  ...
  describe('forwarding', () => {
    ...
    it('forwards the broker payload verbatim to every client of that institution', async () => {
      ...
      handler(boardMonitorTopic(INST_ID), BOARD_MONITOR_PAYLOAD);

      expect(client.sent).toEqual([JSON.stringify(BOARD_MONITOR_PAYLOAD)]);
    });
    ...
    it('discards a message on an unrecognized topic without touching clients', async () => {
      ...
      handler('school-pickup/institution/inst-1/board', BOARD_MONITOR_PAYLOAD);
    ...
    // Carril carries guardian/vehicle data — this test pins that it forwards
    // verbatim, same as BoardGateway does for its own payload (no stripping).
    it('forwards guardian/vehicle fields verbatim, unlike the public board payload', async () => {
      const { gateway, handler } = await subscribedGateway();
      const client = await connect(gateway, authorizedUrl());
