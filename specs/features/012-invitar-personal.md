...
El resultado siempre es una fila en `institution_members`. El camino hacia esa
fila depende de si el correo invitado ya corresponde a un `users` existente o no.

## Entidades involucradas

- `institution_members` (creado)
- `users` (leído; creado con `status = invited` solo en el caso de correo nuevo)
- `institutions` (leído, para autorización multi-tenant)

## Precondiciones

- Quien invita debe ser `institution_members` de la `institution_id` a la que
...
- El rol a asignar debe ser uno de los cuatro valores del enum de
  `institution_members.role`: `admin`, `gate_operator`, `coordinator`, `teacher`
  (ver `specs/entities/institution_member.md`).

## Postcondiciones

Nota importante sobre el estado "Invitado": `institution_members` **no tiene
...
no agrega ninguna columna a `institution_members`.

### Caso (a) — el correo ya es un `users` existente y `active`
- No se crea un `users` nuevo ni se le pide verificar nada.
- Se crea únicamente la fila en `institution_members` vinculando ese `users` con
...
- Se respeta la restricción única `(institution_id, user_id)`: si ese `users` ya
...
### Caso (b) — el correo NO existe como `users`
- Se crea un `users` nuevo con `status = invited` y `password_hash = NULL` (sin
...
- Se crea la fila en `institution_members` con el `role` indicado, vinculada al
  `users` recién creado. Mientras `users.status = invited`, esa persona aparece
...
- No existe un endpoint de reenvío separado para invitaciones de personal.
  Volver a invitar el mismo correo cuando su `users` sigue en `status = invited`
  (y cuyo `institution_members` ya se creó en la primera invitación) se comporta
  como **reenvío**: se genera un token nuevo y se reenvía el correo, **sin**
  crear un segundo `institution_members` (respeta el único
...
- ADR-011 (roles organizacionales de `institution_members`; `admin`,
...
- ADR-022 (punto 1: invitar exige `role = admin`; punto 2: `password_hash`
  nullable para el `users` invitado; punto 5: reenvío vía este mismo endpoint;
...
para el `users` invitado, y el comportamiento de reenvío se resolvieron en
