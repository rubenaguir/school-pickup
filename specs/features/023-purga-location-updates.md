...
- `location_update` (eliminadas las filas fuera de la ventana de retención)
- `pickup_request` (leído: `completed_at` determina la elegibilidad)
...
- Se eliminan las filas de `location_update` cuyo `pickup_request` asociado tiene
...
- No se tocan las `location_update` de trayectos aún activos (`completed_at`
...
- La eliminación es física (no hay borrado lógico para telemetría). El resto del
  `pickup_request` y su `pickup_request_status_history` se conservan: la purga es
...
- ADR-018 (punto 8: retención de 90 días de `location_updates` desde
  `pickup_request.completed_at`; job de limpieza programado).
