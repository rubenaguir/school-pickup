...
para que también sea tutor autorizado (`student_guardians`) de ese mismo alumno,
...
El resultado siempre es una fila en `student_guardians` para ese `(student,
guardián)`. El camino depende de si el correo invitado ya corresponde a un
`users`.

## Entidades involucradas

- `student_guardians` (creado)
- `users` (leído; creado con `status = invited` solo en el caso de correo nuevo)
- `students` (leído, para autorización: el invitador debe ser guardián del alumno)

## Precondiciones

- Quien invita debe ser el `student_guardians` con **`is_primary = true` y
  `status = active`** del `students` al que invita (ADR-023, punto 2): solo el
...
- El `relationship` a asignar debe ser uno de los valores del enum de
  `student_guardians.relationship`: `mother`, `father`, `grandparent`, `driver`,
...
En ambas ramas la fila `student_guardians` nace en `status = invited` y la
...
- No se crea un `users` nuevo.
- Se crea la fila `student_guardians` con `student_id`, `guardian_user_id` (el
...
- Se envía el correo de invitación: la persona debe aceptar para que su
  `student_guardians` pase a `active` (ADR-023 punto 3). Su aceptación **no**
  define contraseña ni verifica correo (el `users` ya está `active`); solo
  transiciona el `student_guardians` (ver feature 016).
...
- Se crea la fila `student_guardians` con `status = invited`, `is_primary = false`
...
- ADR-022 (punto 2: `password_hash` nullable para el `users` invitado).
- ADR-023 (punto 2: solo el guardián `is_primary` invita; punto 3: la invitación
  siempre requiere aceptación, incluso para un `users` ya activo).
...
necesidad de aceptación también en la rama (a) (`users` ya activo) se resolvieron
