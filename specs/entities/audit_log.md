...
- **Contenido de la fila para `pickup_request.delivery_code_mismatched`** (intento fallido de verificar el código de entrega en la consola de puerta; ADR-024 punto 4, ADR-031 puntos 7 y 8): `actor_user_id` = el `institution_members` que tecleó el código; `entity_type = 'pickup_request'`; `entity_id` = el id del `pickup_requests` sobre el que se intentó la entrega; **`metadata = null`**. No se registra el código incorrecto que se tecleó: por minimización de datos (LFPDPPP), el valor forense de la fila está en saber que hubo un intento fallido sobre ese trayecto, cuándo y quién lo hizo — el dígito equivocado no agrega ninguno. Es la única acción de auditoría del slice de `pickup_requests`: las transiciones de estado exitosas se registran en `pickup_request_status_history`, no aquí.
...
- El `verb` va en **participio**, no en sustantivo: `enrollment.approved`, `student_guardian.added`, `institution.suspended`, `pickup_request.delivery_code_mismatched` — no `...delivery_code_mismatch`, que era la forma con la que se acuñó originalmente esta última en ADR-024 y quedó corregida en ADR-031 punto 7 antes de que existiera ninguna fila con el nombre viejo.
...
- ADR-031 (punto 7: `verb` en participio — `pickup_request.delivery_code_mismatched`; punto 8: `entity_type`/`entity_id`/`metadata` de esa acción).
