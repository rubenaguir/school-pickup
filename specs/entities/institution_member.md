...
- `belongsTo User` (`users`) — vía `user_id`.
- Referenciada por `DeliveryPoint.operator_user_id` (indirectamente, vía `user_id` — ver invariante de `delivery_points`).
