...
- `grade_or_group` es editable después de creada la matrícula por dos vías: opcionalmente al aprobar (`PATCH /enrollments/:id/approve`), y mediante `PATCH /enrollments/:id/grade` para matrículas ya `approved` (esta segunda vía exige `status = approved` y, a diferencia de reintentar `approve()`, no reenvía el correo de aprobación). Ver ADR-083.
...
- ADR-083 (`grade_or_group` editable al aprobar y, para matrículas ya `approved`, vía `PATCH /enrollments/:id/grade`).
