...
| `status` | `enum` (`en_route`, `approaching`, `arriving`, `arrived`, `delivered`, `cancelled`) | NOT NULL | mismo enum que `pickup_request.status` |
...
- `changed_by_user_id` es nullable específicamente para representar transiciones automáticas del sistema (ej. `approaching` disparada por el `worker` al entrar al radio de activación, o `arriving` al detectar cercanía por geocerca, sin acción humana).
...
- `status`: mismo dominio que `pickup_request.status` (`en_route`, `approaching`, `arriving`, `arrived`, `delivered`, `cancelled`). Ver `specs/entities/pickup_request.md` para el detalle de transiciones válidas.
