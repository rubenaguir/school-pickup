...
- La entidad TypeORM expone además `institutionId` como propiedad compañera de solo lectura (`insert: false, update: false`), mapeada a la misma columna física que la relación `institution` — no es una columna nueva. Existe para que `InstitutionMembershipGuard` (`@InstitutionResource`, usado por `PATCH /pickup-requests/:id/deliver`) lea el FK como escalar sin tener que cargar la relación completa. Mismo patrón que `institution_member.md`, `delivery_point.md`, `dismissal_window.md`, `dismissal_exception.md` y `enrollment.md`; ver ADR-029.
