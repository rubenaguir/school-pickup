...
| 404 | el `student` no existe |
| 409 | el `user` invitado ya es `student_guardian` de ese alumno (restricción única `(student_id, guardian_user_id)`, en cualquier `status`) |
...
| 409 | se intenta revocar a un `student_guardian` con `is_primary = true` sin reasignar antes la primariedad (ADR-023 punto 5) |
| 409 | se intenta reasignar la primariedad a un `student_guardian` que no está `active` |
| 409 | el `student_guardian` ya está en `status = revoked` (terminal, ADR-018 punto 7) |
